...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # see: http://cs231n.github.io/neural-networks-case-study/#loss

        # note: sum is over K classes
        exp_scores = np.exp(scores)
        probs = exp_scores / np.sum(exp_scores, axis=1, keepdims=True)  # [N x K]

        # for each training example (row), select the probability
        # whose index is given by the associated value in y
        N = X.shape[0]
        winners = probs[range(N), y]

        # compute the loss: average cross-entropy loss and regularization
        correct_logprobs = -np.log(winners)
        ...
        dscores = probs             # N x K
        dscores[range(N), y] -= 1   # subtract 1 from "winners"
        dscores /= N                # and normalise

        grads = {}
        # starting with the loss gradient, propagate back
